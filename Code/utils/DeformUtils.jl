...
module BoundsDefinition
    ...
    function PlotBounds(a, df=DeformationBoundsTable; sgns=[1,1,-1,-1,-1,-1])
        ...
        plts = ParameterRegions(10, 10; a=a, step=0.2)
        ...
        x = plts[3]
        xs = [x[x.<=tri1Int], x[x.>=tri1Int], x[x.<=tri2Int], x[x.>=tri2Int], x, x]
        
        colours = [:blue, :blue, :red, :red, :green, :pink]

        for (i, line) in enumerate(lines)
            if !isnothing(line)
                plot!(xs[i], LineEquation(xs[i], line), label="$i", c=colours[i])
            end
        end
