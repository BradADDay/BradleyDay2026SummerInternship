...
Data from the NuSTAR (Nuclear Spectroscopic Telescope Array) spacecraft @harrison_nuclear_2013 was used, and the dataset used for testing was `nu80402315002B01`. Data loading was handled by `SpectralFitting.jl` through the `OGIPDataset` struct. This takes the spectrum file as an argument and automatically pulls in the background, `.arf`, and `.rmf` files to compute the spectrum.
...
Further reading suggests that it may be possible to constrain the parameters for fitting further. In T. Johannsen's 2013 paper describing the Johannsen metric @johannsen_regular_2013, the following constraints were derived for the deformation parameters:
